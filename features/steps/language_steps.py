...
def step_verify_language_display(context, language):
    ...
    if language.lower() == "ukrainian":
        assert (
            "ua" in current_lang.lower() or "uk" in current_lang.lower()
        ), f"Site not in Ukrainian, current language: {current_lang}"
    elif language.lower() == "russian":
        assert "ru" in current_lang.lower(), f"Site not in Russian, current language: {current_lang}"
...
def step_verify_language_persists(context, language):
    ...
    if language.lower() == "ukrainian":
        ...
        assert "ru" in current_lang.lower(), f"Language did not persist, current: {current_lang}"
...
def step_verify_current_language(context, language):
    ...
    current_lang = context.home_page.get_current_language()
    assert language.lower() in current_lang.lower(), f"Expected language '{language}' but got '{current_lang}'"
